...
@router.put("/me")
def update_me(req: UpdateProfileRequest, user=Depends(verifica_token)):
    conn   = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT id FROM users WHERE email=%s AND id!=%s", (req.email, user["sub"]))
    if cursor.fetchone():
        cursor.close(); conn.close()
        raise HTTPException(400, "Emailul este deja folosit de alt cont.")
    cursor.execute(
        "UPDATE users SET name=%s, email=%s WHERE id=%s",
        (req.name, req.email, user["sub"])
    )
    conn.commit(); cursor.close(); conn.close()
...
def user_from_token(token: str):
    conn   = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT name, email, reset_expires FROM users WHERE reset_token=%s", (token,))
    row = cursor.fetchone()
    cursor.close(); conn.close()
    if not row:
        raise HTTPException(400, "Token invalid.")
    ...
    if expires.tzinfo is None:
        expires = expires.replace(tzinfo=timezone.utc)
    if datetime.now(timezone.utc) > expires:
        raise HTTPException(400, "Token expirat.")
