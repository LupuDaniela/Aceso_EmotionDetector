...
class AcesoPipeline:
    ...
    def analizeaza(self, text: str, salveaza: bool = True, user_id: int = None) -> dict:
    ...
    def _salveaza(self, message, emotie, scor, toate_scorurile, diade, user_id=None):
        try:
            conn = get_conn(); cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO conversations
                    (user_id, message, emotie_dominanta, scor_dominant,
                     toate_scorurile, diade_detectate)
                VALUES (%s,%s,%s,%s,%s,%s)
            """, (user_id, message, emotie, scor,
                  Json(toate_scorurile), Json(diade)))
